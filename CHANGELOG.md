...
滚动命名与定时滚动行为增强（与 retrieval-engine / aegis 的 LOG_CALL_INFO 需求对齐）。

新增能力 / 行为变更：

- **[命名] 支持“序号始终存在”的命名模式**：当 `NamePattern` 内嵌 `{seq}` 时，当天首个文件即为 `.01.`（如 `LOG_CALL_INFO.2026-09-12-18.01.aegis9092.log`），同小时内文件超过 `MaxSize` 时递增到 `.02.`/`.03.`…，**跨小时切到新文件时序号重置为 `.01.`**。该行为对默认（无 `{seq}`）命名模式完全透明：首文件仍无序号、仅在滚动后出现 `.01.`，与旧版本一致。
- **[命名] 重启续接更稳健**：全新启动（当前时段无文件）时，序号起点由 `NamePattern` 是否含 `{seq}` 决定（含则 1、否则 0），避免旧逻辑在序号命名模式下生成非预期的 `.00.`。
- **[定时滚动] `RotationInterval = 0` 现表示“禁用定时滚动”**（此前 0 会被强制回退为默认 `DefaultRotationInterval`=1h）。即：仅按 `DateLayout`（如按小时 `2006-01-02-15`）与文件大小 `MaxSize` 两种规则切分新文件；设置非 0 时长（如 `1h`）才额外启用定时滚动。**注意**：此变更使默认值从“每小时定时滚动”变为“禁用定时滚动（仅按 DateLayout + 大小）”，依赖旧默认值的调用方需显式设置 `RotationInterval`。
